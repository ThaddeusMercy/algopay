...
export const fetchCryptoPrice = async (cryptoName: string): Promise<PriceData | null> => {
  try {
...
export const fetchHistoricalData = async (cryptoName: string, days: number = 30): Promise<HistoricalData | null> => {
  try {
    const cleanName = cryptoName.toLowerCase().trim();
    const cryptoId = cryptoIds[cleanName] || cleanName;
...
export const fetchMultipleCryptoPrices = async (cryptoNames: string[]): Promise<{ [key: string]: PriceData }> => {
  try {
    const ids = cryptoNames.map(name => {
...
export const formatPriceResponse = (priceData: PriceData, cryptoName: string): string => {
...
🪙 **Supply:** ${(priceData.supply / 1e9).toFixed(2)}B tokens`;
  ...
  if (priceData.maxSupply) {
    response += `\n⚡ **Max Supply:** ${(priceData.maxSupply / 1e9).toFixed(1)}B`;
  }
  ...
  if (priceData.ath) {
    ...
    response += `\n📉 **From ATH:** ${priceData.athChangePercentage?.toFixed(1)}%`;
  }
  ...
  if (priceData.atl) {
    ...
    response += `\n📈 **From ATL:** ${priceData.atlChangePercentage?.toFixed(1)}%`;
  }
...
export const getAdvancedAnalysis = async (cryptoName: string): Promise<string> => {
  try {
...
• Trend: ${analysis.trend.toUpperCase()} ${analysis.trend === 'bullish' ? '🟢' : analysis.trend === 'bearish' ? '🔴' : '🟡'}
• Volatility: ${analysis.volatility.toUpperCase()} ${analysis.volatility === 'high' ? '⚠️' : '✅'}
...
export const getMarketSummary = async (): Promise<string> => {
  try {
    const topCryptos = await fetchMultipleCryptoPrices(['bitcoin', 'ethereum', 'algorand', 'cardano', 'solana']);
