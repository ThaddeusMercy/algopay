...
import { connectWallet, disconnectWallet, reconnectWallet } from '../utils/algorand';
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ companyName }) => {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return savedTab && ['dashboard', 'employees', 'bulk-transfer', 'ai-assistant-chat', 'ai-assistant-history', 'settings'].includes(savedTab) 
      ? savedTab 
      : 'dashboard';
  });
  ...
  // Reset notification flag when user changes
  useEffect(() => {
    if (!user) {
  ...
  return (
    <div className="h-screen flex">
      ...
      {/* Main Content */}
      <div className={`flex-1 flex flex-col h-screen bg-white text-gray-900 transition-all duration-300 ${getMainContentMargin()}`}>
        {/* Fixed Top Bar */}
      ...
      {isConnecting && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white border border-gray-200 rounded-xl p-6 sm:p-8 text-center">
            ...
            <div className="text-gray-900 font-medium text-sm sm:text-base">Connecting to Pera Wallet...</div>
            <div className="text-gray-600 text-xs sm:text-sm mt-2">Please confirm in your wallet app</div>
          </div>
        </div>
      )}
    </div>
