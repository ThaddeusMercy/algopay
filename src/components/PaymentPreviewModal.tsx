...
import { sendBulkPayment, isWalletConnected, getConnectedAccount, formatAddress } from '../utils/algorand';
...
export const PaymentPreviewModal: React.FC<PaymentPreviewModalProps> = ({
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const recipientsData = employeesToPay.map(emp => {
        console.log(`Employee ${emp.name}: wallet_address = ${emp.wallet_address}, amount = ${emp.amount}`);
        
        // Validate each employee has required fields
        if (!emp.wallet_address || !emp.amount) {
          throw new Error(`Invalid data for employee ${emp.name}: missing wallet address or amount`);
        }
      ...
      if (result.success) {
        ...
        try {
          const emailDataList: PaymentEmailData[] = employeesToPay.map(employee => ({
            ...
            companyName: 'Algopay', // You can make this dynamic later
  ...
  const viewOnExplorer = () => {
    if (paymentResult?.txHash) {
      window.open(`https://testnet.algoexplorer.io/tx/${paymentResult.txHash}`, '_blank');
    }
  };
            ...
            <div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-gray-900">{networkFees.toFixed(3)} ALGO</span>
                </div>
                ...
                <div className="border-t border-gray-200 pt-4">
                  <div className="flex items-center justify-between text-lg">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-600">+ {networkFees.toFixed(3)} ALGO fees</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 mt-4">
                  ...
                  <div className="text-xs text-green-700">
                    All payments are processed on the Algorand blockchain with cryptographic security.
                  </div>
                </div>
              </div>
            </div>
