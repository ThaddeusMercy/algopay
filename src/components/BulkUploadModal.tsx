...
export const BulkUploadModal: React.FC<BulkUploadModalProps> = ({
  ...
  const [parsedData, setParsedData] = useState<Omit<Employee, 'id' | 'user_id' | 'created_at' | 'updated_at'>[]>([]);
  ...
  const parseCSV = (csvText: string): { data: Omit<Employee, 'id' | 'user_id' | 'created_at' | 'updated_at'>[]; errors: string[] } => {
    ...
    const expectedHeaders = [
      ...
      'wallet_address', 'join_date', 'status'
    ];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        if (!employee.wallet_address || employee.wallet_address.length < 58) {
          errors.push(`Row ${i + 1}: Valid Algorand address is required`);
        }
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      >
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
          <div>
            <h2 className="text-lg sm:text-2xl font-bold text-gray-900">Bulk Upload Employees</h2>
            <p className="text-gray-600 text-xs sm:text-sm">Upload multiple employees using a CSV file</p>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  <h3 className="font-medium text-blue-800 text-sm sm:text-base">Download Template</h3>
                  <p className="text-xs sm:text-sm text-blue-700">Get the CSV template with sample data</p>
                </div>
              </div>
              <button
          ...
          {isProcessing && (
            <div className="flex items-center justify-center space-x-2 sm:space-x-3 py-3 sm:py-4">
              ...
              <span className="text-gray-700 text-sm sm:text-base">Processing CSV file...</span>
            </div>
          )}
          ...
          {isUploading && (
            <div className="flex items-center justify-center space-x-2 sm:space-x-3 py-3 sm:py-4">
              ...
              <span className="text-gray-700 text-sm sm:text-base">Uploading employees...</span>
            </div>
          )}
          ...
          {errors.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4">
              <div className="flex items-center space-x-2 mb-2 sm:mb-3">
                ...
                <h3 className="font-medium text-red-800 text-sm sm:text-base">Validation Errors</h3>
              </div>
              <div className="space-y-1 max-h-24 sm:max-h-32 overflow-y-auto">
                {errors.map((error, index) => (
                  <p key={index} className="text-xs sm:text-sm text-red-700">• {error}</p>
                ))}
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 gap-2">
                  {parsedData.slice(0, 6).map((employee, index) => (
                    <div key={index} className="flex items-center space-x-2 text-xs sm:text-sm">
                      ...
                      <span className="text-gray-700 truncate">{employee.name}</span>
                      <span className="text-gray-500 hidden sm:inline">({employee.designation})</span>
                    </div>
                  ))}
                  {parsedData.length > 6 && (
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 sm:pt-6">
            ...
            <button
              ...
              className={`flex-1 bg-gray-900 hover:bg-gray-800 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-lg transition-colors text-sm sm:text-base ${
                parsedData.length === 0 || errors.length > 0 || isUploading ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
              {isUploading ? 'Uploading...' : `Upload ${parsedData.length} Employees`}
            </button>
          </div>
        </div>
      </motion.div>
